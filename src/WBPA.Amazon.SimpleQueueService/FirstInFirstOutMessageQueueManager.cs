...
{
    /// <summary>
    /// Provides ways for managing messages on first-in-first-out AWS SQS. This class cannot be inherited.
    /// </summary>
    /// <seealso cref="MessageQueueManager" />
    public sealed class FirstInFirstOutMessageQueueManager : MessageQueueManager
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="FirstInFirstOutMessageQueueManager"/> class.
        /// </summary>
        ...
        public FirstInFirstOutMessageQueueManager(Uri queueEndpoint, AWSCredentials credentials, Action<ManagerOptions> setup = null) : base(queueEndpoint, credentials, setup)
        {
        }
        ...
        private async Task<SendMessageBatchResponse> SendBatchCoreAsync(IEnumerable<string> messages, FirstInFirstOutQueueSendOptions options, int initialCounter = 0)
        {
            var entries = BatchUtility.CreateBatchRequestEntries(messages, (message, counter) => new SendMessageBatchRequestEntry(options.BatchRequestIdGenerator?.Invoke(counter) ?? "Entry{0}".FormatWith(counter), message)
            {
                MessageAttributes = options.MessageAttributes,
                MessageGroupId = options.MessageGroupId,
                MessageDeduplicationId = options.BatchMessageDeduplicationIdGenerator?.Invoke(counter)
        ...
        /// <returns>The task object representing the asynchronous operation.</returns>
        /// <remarks>Messages are delivered in partitions of ten through <see cref="SendBatchAsync(IEnumerable{string})"/>.</remarks>
        public async Task<IEnumerable<SendMessageBatchResponse>> SendManyBatchAsync(IEnumerable<string> messages, Action<FirstInFirstOutQueueSendOptions> setup)
        {
            Validator.ThrowIfNull(messages, nameof(messages));
            var options = setup.ConfigureOptions();
        ...
        /// <returns>The task object representing the asynchronous operation.</returns>
        /// <remarks>Messages are retrieved and aggregated by calling <see cref="ReceiveAsync()"/>.</remarks>
        public async Task<IEnumerable<ReceiveMessageResponse>> ReceiveManyAsync(int approximateCount, Action<FirstInFirstOutQueueReceiveOptions> setup)
        {
